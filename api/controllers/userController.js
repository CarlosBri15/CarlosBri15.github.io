...
function getUser(req, res) {
  ...
  User.findById(userId, (error, user) => {
    if (error) return res.status(404).send({ message: 'User not found', error });
...
function createUser(req, res) {
  ...
  user.save((error, newUser) => {
    ...
    return res.status(200).send({ message: 'User saved', newUser, token: service.createToken(user) });
  });
}
...
function replaceUser(req, res) {
  ...
  User.findById(userId, (err, user) => {
    if (err) return res.status(404).send({ message: 'No user found to be replace', err });
...
function login(req, res) {
  ...
  User.findOne({ email }, (err, user) => {
    ...
    bcrypt.compare(password, user.password, (error, same) => {
      ...
      if (same) return res.status(200).send({ message: 'Correct password', token: service.createToken(user) });
